...
async function sendGroupAnnouncement(groupId, title, content) {
    try {
        ...
        // Try v2 endpoint
        const res = await fetch(`https://groups.roblox.com/v2/groups/${groupId}/announcements`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Cookie': `.ROBLOSECURITY=${COOKIE()}`,
        ...
        if (!res.ok) {
            ...
            const res2 = await fetch(`https://groups.roblox.com/v2/groups/${groupId}/announcements`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Cookie': `.ROBLOSECURITY=${COOKIE()}`,
            ...
            console.log('📢 Alt format status:', res2.status);
            console.log('📢 Alt format response:', text2);
            return res2.ok;
        }
