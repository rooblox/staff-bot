...
// Main server role (left) -> Mod server role (right)
// If user HAS the mod server role (right), give them the main role (left)
// If user is MISSING the mod server role (right), remove the main role (left)
// Leadership roles are manual — bot never touches them
const MOD_ROLE_MAP = {
    '1464028063909019882': '1372326605518802955', // Moderation Team (main) <- mod role
    '1464027433476030690': '1373135168784695416', // Trial Moderator (main) <- mod role
    '1464027469999902812': ['1373135108092989440', '1373134928337965056'], // Junior/Moderator (main) <- either
    '1464027539537137938': ['1373134577174057080', '1373134536111820881'], // Senior/Head Mod (main) <- either
    '1464028279131340843': '1373551504773877790', // Moderation Department (main) <- mod role
    '1434563855354167358': '1434612807223218216', // Human Resources Department (main) <- mod role
};
    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            for (const mainMember of membersToSync) {
                ...
                const modMember = modGuild.members.cache.get(mainMember.id);
                if (!modMember) {
                ...
                for (const [mainRoleId, modRoleIds] of Object.entries(MOD_ROLE_MAP)) {
                    if (MANUAL_MAIN_ROLES.has(mainRoleId)) continue;
                    const modRoles = Array.isArray(modRoleIds) ? modRoleIds : [modRoleIds];
                    const hasInMod = modRoles.some(id => modMember.roles.cache.has(id));
