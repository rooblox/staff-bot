    ...
    try {
        ...
        for (const ticket of openTickets) {
            try {
                const channel = await member.guild.channels.fetch(ticket.channelId).catch(() => null);
                if (!channel) continue;
                await channel.send({ files: ['https://cdn.discordapp.com/attachments/620024668673409069/1521631185749676152/w2jpc0AAAAGSURBVAMAewXVEO1vryYAAAAASUVORK5CYII.png?ex=6a4588f9&is=6a443779&hm=bfb9a11c7f931f578c72ebf81c51e1729549c85803af6ff9e384ea18fc909295&'] });
                await channel.send({
                    embeds: [new EmbedBuilder()
                        .setTitle('🚪 Ticket Creator Left the Server')
                        .setDescription(`**${member.user.tag}** has left the server.\n\nThis ticket was opened by them. A staff member can close it below if it is no longer needed.`)
